...
export const PhotosCarousel = ({
	...
	file: File | { asset_id: string; public_id: string }
}) => {
	if (file && 'asset_id' in file && 'public_id' in file) {
		...
		return (
			...
			>
				<CloudinaryImageComponent imgSrc={file.public_id as string} />
			</div>
		)
		...
		// file is a dummy div
		return (
			<div className="aspect-h-4 aspect-w-4 flex items-center justify-center rounded-lg border border-dashed border-slate-900/25">
				{file}
			</div>
		)
	}
